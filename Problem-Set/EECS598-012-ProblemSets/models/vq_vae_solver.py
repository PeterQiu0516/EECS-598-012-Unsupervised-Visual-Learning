...
def eval_loss(model, data_loader, quiet):
    ...
    with torch.no_grad():
        for x in data_loader:
            ...
            loss_dict = vq_vae_loss(x_tilde, diff, x)
            for k, v in loss_dict.items():
                total_losses[k] = total_losses.get(
                    k, 0) + v.item() * x.shape[0]
...
def train_epochs_pixcnn(model, train_loader, test_loader, train_args, quiet=False):
    ...
    for epoch in range(epochs):
        model.train()
        train_loss = train_pixcnn(
            model, train_loader, optimizer, epoch, quiet, grad_clip)
...
def train_vq_vae(train_data, test_data):
    ...
    # print(model.codebook.embedding.weight.max())
    # print(model.codebook.embedding.weight.mean())
    # print(model.codebook.embedding.weight.min())
    # input()
    ...
    prior = PixelCNN(code_size=128, input_shape=(
        8, 8), dim=128, n_layers=10).cuda()
    prior_train_data, prior_test_data = create_prior_dataset(
        train_loader), create_prior_dataset(test_loader)
    ...
    with torch.no_grad():
        z = model.encode_code(x)
        x_recon = model.decode_code(z)  # [0, 1]
    ...
    reconstructions = np.stack(
        (x, x_recon), axis=1).reshape((-1, 32, 32, 3)) * 255

    # vqvae_train_losses, vqvae_test_losses, prior_train_losses, prior_test_losses = 0.0, 0.0, 0.0, 0.0
