...
def train(model, train_loader, optimizer, epoch, ttl_epochs=0, grad_clip=None, quiet=False):
    ...
    for batch in train_loader:
        ...
        """
        # print(feat_img.shape) # B x 256 x D
        # print(feat_aud.shape) # B x D
        # input()

        # print(feat_img.shape) # B x 256 x D
        # print(feat_aud.shape) # B x D
        # input()
        # B x 256 x B sim of each patch of each img for each audio
        sim = torch.matmul(feat_img, feat_aud)
        # sim = torch.zeros((feat_img.shape[0], feat_img.shape[1], 1)) # B x 256 x 1
        # for b in range(feat_img.shape[0]):
        #     sim[b] = torch.matmul(feat_img[b], feat_aud[b].reshape(-1, 1)) # 256 x 1
        # # print(sim.shape)
        tau = 0.1
        if max_mode:
            # B x B, max of sim across patches of each img for each audio
            sim_max = sim.amax(dim=1) / tau
            # print(sim_max)
            loss = torch.diag(- F.log_softmax(sim_max, dim=0)).mean()
            # print(loss)
            # print(loss.shape)
            # print(loss.sum())
            # print(torch.exp(-loss).sum())
            # input()
        else:
            # B x B, mean of sim across patches of each img for each audio
            sim_avg = sim.mean(dim=1) / tau
            # print(sim_avg)
            loss = torch.diag(-F.log_softmax(sim_avg, dim=1)).mean()
            # print(loss)
            # print(loss.shape)
            # print(loss.sum())
            # print(torch.exp(-loss).sum())
...
def train_epochs(model, train_loader, test_loader, train_args, exp_path=None, quiet=False):
    ...
    for epoch in range(epochs):
        ...
        train_losses.extend(train(model, train_loader, optimizer,
                                  epoch, ttl_epochs=epochs, grad_clip=None, quiet=quiet))
        test_loss = eval_loss(model, test_loader,
                              ttl_epochs=epochs, epoch=epoch, quiet=quiet)
        test_losses.append(test_loss)
        if not quiet:
            print(f'Epoch {epoch}, Test loss {test_loss:.4f}')
        ssl_results = loc_sound_source(test_loader, model)
        ssl_results = np.array(
            ssl_results, dtype='uint8').transpose(0, 3, 1, 2)
...
def loc_sound_source(data_loader, model):
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ...
            sim_up = nn.functional.interpolate(sim, size=(
                (256, 256)), mode='bilinear', align_corners=False).squeeze()
