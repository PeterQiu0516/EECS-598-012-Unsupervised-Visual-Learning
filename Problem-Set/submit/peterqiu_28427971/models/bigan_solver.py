...
class Solver(object):
    ...
    def build(self):
        ...
        self.g_scheduler = torch.optim.lr_scheduler.LambdaLR(self.g_optimizer,
                                                             lambda epoch: (
                                                                 self.n_epochs - epoch) / self.n_epochs,
                                                             last_epoch=-1)
        ...
        self.linear_optimizer = torch.optim.Adam(
            self.linear.parameters(), lr=1e-3)
    ...
    def train_bigan(self):
        ...
        for epoch_i in tqdm_notebook(range(self.n_epochs), desc='Epoch'):
            ...
            for batch_i, (x, y) in enumerate(tqdm_notebook(self.train_loader, desc='Batch', leave=False)):
                ...
                # --------------------------------------------------- #
                x = x.view(x.shape[0], -1)  # 128 x 784

                # Generator
                z_fake1 = torch.randn((x.shape[0], self.latent_dim)).cuda()
                x_fake1 = self.g(z_fake1).view(x.shape[0], -1)  # [-1, 1]

                # Encoder
                z1 = self.e(x)

                # Discrimiator
                d_g1 = self.d(z_fake1, x_fake1)
                d_e1 = self.d(z1, x)

                EPS = 1e-8
                d_loss = -torch.mean(torch.log(d_e1+EPS) +
                                     torch.log(1-d_g1+EPS))
                ...
                # Generator
                z_fake2 = torch.randn((x.shape[0], self.latent_dim)).cuda()
                x_fake2 = self.g(z_fake2).view(x.shape[0], -1)  # [-1, 1]

                # Encoder
                z2 = self.e(x)

                # Discrimiator
                d_g2 = self.d(z_fake2, x_fake2)
                d_e2 = self.d(z2, x)
                g_loss = torch.mean(torch.log(d_e2+EPS) +
                                    torch.log(1-d_g2+EPS))
...
def train_bigan(train_data, test_data):
    ...
    # print(samples.shape
    # sample from BiGAN model, 100 x 1 x 28 x 28 -> 100 x 28 x 28 x 1
    samples = solver.sample(100).transpose(0, 2, 3, 1) * 0.5 + 0.5
    train_images = train_data.data[:20].reshape(
        20, 1, 28, 28) / 255.0  # [0, 1]
    train_img_tensor = train_images.float().cuda() * 2 - 1  # [-1, 1]
    # get original and reconstructed images pairs from the trained model, 20 x 28 x 28 x 1
    recons = solver.get_reconstructions(train_img_tensor) * 0.5 + 0.5
    reconstructions = np.concatenate(
        [train_images, recons], axis=0).transpose(0, 2, 3, 1)
